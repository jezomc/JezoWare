...
var snowball_collected = 0 # just keeping track of garlic collected
...
func _process(delta: float) -> void: # running every frame brochacho
	...
	if snowball_collected == 3: # the double equals is just an argument asking if it's the same, with "=" it'll give an error
	...
	if timer_end: # if the timer does end...
		...
		Global.hearts -= 1 # lose ur lives
		SceneTransition.change_scene_to_file("res://scenes/timer_screen.tscn")# back to intermission
		

func snowball_collect() -> void: # cool function that you connect to those garlics
	snowball_collected = snowball_collected +1
